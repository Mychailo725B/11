...
int main() {
    ...
    // 1. Створення динамічних масивів за допомогою покажчиків
    int* p = new int[n];
    int* q = new int[n];

    // Заповнення масивів випадковими числами від -50 до 50 та вивід на екран
    cout << "\nМасив p: ";
    for (int i = 0; i < n; i++) {
        p[i] = rand() % 101 - 50;
        cout << p[i] << " ";
    ...
    // 2. Підрахунок від'ємних елементів
    int countP = 0;
    int countQ = 0;
    ...
    // 3. Порівняння результатів та виведення відповіді
    ...
    // 4. Обов'язкове звільнення виділеної динамічної пам'яті
    delete[] p;
    delete[] q;
